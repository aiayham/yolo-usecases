import math
import os
from ultralytics import YOLO
import cv2
import cvzone
import torch
from sort import *
import numpy as np
import arabic_reshaper
from bidi.algorithm import get_display
from PIL import ImageFont, ImageDraw, Image

print(torch.cuda.is_available())
print(torch.cuda.get_device_name(0) if torch.cuda.is_available() else "No GPU")

# ── Arabic font ────────────────────────────────────────────────────────────────
_FONT_PATHS = [
    "C:/Windows/Fonts/tahoma.ttf",
    "C:/Windows/Fonts/arial.ttf",
    "C:/Windows/Fonts/segoeui.ttf",
]
ARABIC_FONT = next((p for p in _FONT_PATHS if os.path.exists(p)), None)
if not ARABIC_FONT:
    raise FileNotFoundError("Arabic font not found. Tahoma / Arial / Segoe UI required.")

_font_cache = {}

def get_font(size):
    if size not in _font_cache:
        _font_cache[size] = ImageFont.truetype(ARABIC_FONT, size)
    return _font_cache[size]

def ar(text):
    """Reshape + BiDi an Arabic string so PIL renders it correctly."""
    return get_display(arabic_reshaper.reshape(str(text)))


# ── Model ──────────────────────────────────────────────────────────────────────
model = YOLO("D:/yolo/yolo_weights/yolov8x-worldv2.pt")
model.to("cuda")

TRACK_CLASSES = ["person"]
model.set_classes(TRACK_CLASSES)
classNames = model.names

# ── Arabic labels & colours ────────────────────────────────────────────────────
AR = {
    "person":  "شخص",
    "unknown": "غير محدد",
}
PERSON_RGB  = ( 48, 176, 199)   # teal — PIL (R,G,B)
PERSON_BGR  = (199, 176,  48)   # teal — OpenCV (B,G,R)
CROWD_RGB   = (255, 149,   0)   # orange — PIL  (crowded state)
CROWD_BGR   = (  0, 149, 255)   # orange — OpenCV


# ── Video & tracker ────────────────────────────────────────────────────────────
cap          = cv2.VideoCapture("D:/yolo/videos/2.mp4")
tracker      = Sort(max_age=20, min_hits=2, iou_threshold=0.3)
id_best      = {}   # {id: {"label": str, "conf": float}}
id_last_seen = {}   # {id: frame_index}
frame_idx    = 0

STALE_FRAMES = 30   # remove IDs not seen for 30 frames (tune: 20-60)
CROWD_LIMIT  = 75   # active-person count that triggers the crowded alert

# ── Safe-zone HUD geometry (TikTok / Instagram Reels = 720×1280 display) ──────
#   Top danger    0–180 px  : status bar, username, follow button
#   Bottom danger 960–1280 px: caption, like/comment/share, seekbar
#   Right danger  590–720 px : action buttons column
orig_w  = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
orig_h  = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
_sx, _sy = orig_w / 720, orig_h / 1280

HUD_X    = max(16, int(28  * _sx))   # left margin in original-frame px
HUD_Y    = max(10, int(220 * _sy))   # start below top danger zone
ROW_H    = max(40, int(74  * _sy))   # per-row height
FS_TITLE = max(24, int(38  * _sy))   # title font size
FS_ROW   = max(18, int(30  * _sy))   # count-row font size
FS_LABEL = max(14, int(20  * _sy))   # per-person track label (kept small — many people)
BORDER_T = max(6,  int(12  * _sx))   # crowd-border thickness



# ── IoU helper ─────────────────────────────────────────────────────────────────
def box_iou(a, b):
    ix1, iy1 = max(a[0], b[0]), max(a[1], b[1])
    ix2, iy2 = min(a[2], b[2]), min(a[3], b[3])
    inter = max(0, ix2 - ix1) * max(0, iy2 - iy1)
    if not inter:
        return 0.0
    return inter / ((a[2]-a[0])*(a[3]-a[1]) + (b[2]-b[0])*(b[3]-b[1]) - inter + 1e-9)


# ── Frame-level Arabic text batcher ───────────────────────────────────────────
class FrameAR:
    """
    Collect all Arabic draw calls, then composite them in one PIL round-trip
    per frame.
    """

    def __init__(self):
        self._q = []

    def text(self, txt, pos, size=28, fg=(255, 255, 255),
             bg=(15, 15, 15, 210), pad=8, radius=8):
        self._q.append(("text", txt, pos, size, fg, bg, pad, radius))

    def circle(self, center, radius, color_rgba):
        self._q.append(("circle", center, radius, color_rgba))

    def flush(self, img):
        if not self._q:
            return img
        pil = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).convert("RGBA")
        ov  = Image.new("RGBA", pil.size, (0, 0, 0, 0))
        d   = ImageDraw.Draw(ov)
        for item in self._q:
            if item[0] == "text":
                _, txt, (x, y), size, fg, bg, pad, r = item
                shaped = ar(txt)
                font   = get_font(size)
                bb     = d.textbbox((0, 0), shaped, font=font)
                tw, th = bb[2] - bb[0], bb[3] - bb[1]
                d.rounded_rectangle(
                    [x - pad, y - pad, x + tw + pad, y + th + pad],
                    radius=r, fill=bg,
                )
                d.text((x, y), shaped, font=font, fill=fg)
            elif item[0] == "circle":
                _, (cx, cy), rad, col = item
                d.ellipse([cx - rad, cy - rad, cx + rad, cy + rad], fill=col)
        self._q.clear()
        return cv2.cvtColor(
            np.array(Image.alpha_composite(pil, ov).convert("RGB")),
            cv2.COLOR_RGB2BGR,
        )


# ── Crowd-border overlay ───────────────────────────────────────────────────────
def draw_crowd_border(img):
    """Draw a layered orange border to signal crowding."""
    h, w = img.shape[:2]
    overlay = img.copy()
    cv2.rectangle(overlay, (0, 0), (w - 1, h - 1), CROWD_BGR, BORDER_T * 3)
    cv2.addWeighted(overlay, 0.4, img, 0.6, 0, img)
    cv2.rectangle(img, (0, 0), (w - 1, h - 1), CROWD_BGR, BORDER_T)


# ── HUD layout ─────────────────────────────────────────────────────────────────
def queue_hud(fa: FrameAR, active_count: int):
    """Add the crowd-count panel (and optional crowded alert) to the batch."""
    # Title
    fa.text("رصد الحشود", (HUD_X, HUD_Y), size=FS_TITLE,
            fg=(255, 255, 255), bg=(0, 0, 0, 210), pad=14, radius=12)

    # Active-person count row
    y = HUD_Y + int(FS_TITLE * 1.9)
    box_color = CROWD_RGB if active_count >= CROWD_LIMIT else PERSON_RGB
    fa.circle((HUD_X + 24, y + FS_ROW // 2), 11, (*box_color, 255))
    fa.text(
        f"أشخاص نشطون   {active_count}",
        (HUD_X + 44, y), size=FS_ROW,
        fg=box_color, bg=(5, 5, 5, 195), pad=10, radius=8,
    )

    # Crowded alert — shown below the count row
    if active_count >= CROWD_LIMIT:
        fa.text(
            "تحذير: ازدحام!",
            (HUD_X, y + ROW_H + 8), size=FS_ROW,
            fg=(255, 255, 255),
            bg=(180, 80, 0, 240),
            pad=12, radius=10,
        )


# ── Main loop ──────────────────────────────────────────────────────────────────
while True:
    frame_idx += 1
    success, img = cap.read()
    if not success:
        break

    fa         = FrameAR()
    results    = model(img, stream=True, imgsz=736, device=0)
    detections = np.empty((0, 5))
    det_meta   = []

    for r in results:
        for box in r.boxes:
            x1, y1, x2, y2 = (int(v) for v in box.xyxy[0])
            conf  = math.ceil(box.conf[0] * 100) / 100
            cls   = int(box.cls[0])
            label = TRACK_CLASSES[cls] if 0 <= cls < len(TRACK_CLASSES) else "unknown"

            cvzone.cornerRect(img, (x1, y1, x2 - x1, y2 - y1),
                              colorR=PERSON_BGR, colorC=PERSON_BGR)

            detections = np.vstack((detections, [x1, y1, x2, y2, conf]))
            det_meta.append((x1, y1, x2, y2, float(conf), label))

    resultTracker = tracker.update(detections)

    # Prune tracks not seen for STALE_FRAMES frames
    stale_ids = [i for i, last in id_last_seen.items() if frame_idx - last > STALE_FRAMES]
    for i in stale_ids:
        id_last_seen.pop(i, None)
        id_best.pop(i, None)

    # Active count: snapshot after stale pruning, before this frame's updates
    active_count = len(id_last_seen)

    # Crowd border (drawn on img directly, before PIL pass)
    if active_count >= CROWD_LIMIT:
        draw_crowd_border(img)

    for result in resultTracker:
        x1, y1, x2, y2, Id = result
        x1, y1, x2, y2, Id = int(x1), int(y1), int(x2), int(y2), int(Id)
        id_last_seen[Id] = frame_idx

        best_label, best_conf, best_v = "unknown", 0.0, 0.0
        for dx1, dy1, dx2, dy2, dconf, dlabel in det_meta:
            v = box_iou((x1, y1, x2, y2), (dx1, dy1, dx2, dy2))
            if v > best_v:
                best_v, best_label, best_conf = v, dlabel, dconf

        prev = id_best.get(Id)
        if prev is None or best_conf > prev["conf"]:
            id_best[Id] = {"label": best_label, "conf": best_conf}

        stable = id_best[Id]["label"]
        box_color = CROWD_BGR if active_count >= CROWD_LIMIT else PERSON_BGR
        cv2.rectangle(img, (x1, y1), (x2, y2), box_color, 2)

        fa.text(
            f"{AR.get(stable, stable)}  #{Id}",
            (x1, max(30, y1 - 30)),
            size=FS_LABEL,
            fg=(255, 255, 255),
            bg=(*(CROWD_RGB if active_count >= CROWD_LIMIT else PERSON_RGB), 200),
            pad=5,
            radius=5,
        )

    # HUD queued last so it composites on top of all person labels
    queue_hud(fa, active_count)

    img     = fa.flush(img)
    display = cv2.resize(img, (720, 1280))
    cv2.imshow("Image", display)
    cv2.waitKey(1)
