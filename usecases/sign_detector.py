...
TRACK_CLASSES = ["traffic_sign"]
model.set_classes(TRACK_CLASSES)
classNames = model.names

# ── Arabic labels & colours ────────────────────────────────────────────────────
AR = {
    "traffic_sign": "إشارة مرور",
    "unknown":      "غير محدد",
}
SIGN_RGB = (255, 196,   0)   # amber — PIL (R,G,B)
SIGN_BGR = (  0, 196, 255)   # amber — OpenCV (B,G,R)


# ── Video, mask & tracker ──────────────────────────────────────────────────────
cap  = cv2.VideoCapture("D:/yolo/videos/3.mp4")
mask = cv2.imread("D:/yolo/videos/3mask.png")

# Ensure mask matches frame size
success, _test = cap.read()
cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
mask = cv2.resize(mask, (_test.shape[1], _test.shape[0]))

tracker      = Sort(max_age=20, min_hits=2, iou_threshold=0.3)
id_best      = {}   # {id: {"label": str, "conf": float}}
id_last_seen = {}   # {id: frame_idx}
frame_idx    = 0
STALE_FRAMES = 15   # tune: 10-40 depending on FPS / occlusion
...
FS_LABEL = max(18, int(26  * _sy))   # per-detection label font size
...
def queue_hud(fa: FrameAR, active_count: int):
    """Add the sign-count panel to the batch (drawn during fa.flush)."""
    fa.text("رصد الإشارات المرورية", (HUD_X, HUD_Y), size=FS_TITLE,
            fg=(255, 255, 255), bg=(0, 0, 0, 210), pad=14, radius=12)

    y = HUD_Y + int(FS_TITLE * 1.9)
    fa.circle((HUD_X + 24, y + FS_ROW // 2), 11, (*SIGN_RGB, 255))
    fa.text(
        f"إشارات نشطة   {active_count}",
        (HUD_X + 44, y), size=FS_ROW,
        fg=SIGN_RGB, bg=(5, 5, 5, 195), pad=10, radius=8,
    )
...
while True:
    ...
    for r in results:
        for box in r.boxes:
            ...
            cvzone.cornerRect(img, (x1, y1, x2 - x1, y2 - y1),
                              colorR=SIGN_BGR, colorC=SIGN_BGR)
    ...
    # Active count is snapshot after stale pruning, before this frame's updates
    ...
    for result in resultTracker:
        ...
        cv2.rectangle(img, (x1, y1), (x2, y2), SIGN_BGR, 2)
